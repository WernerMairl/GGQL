...
{
    internal sealed class GuardAssertException : InvalidOperationException
    {
    ...
    [System.Diagnostics.DebuggerNonUserCodeAttribute(), System.Diagnostics.DebuggerStepThrough()]
     internal static class Guard //WM 06/2017 public because used in other Assemblies that are part of the SAME Solution!
    {
        ...
        [System.Diagnostics.DebuggerNonUserCodeAttribute(), System.Diagnostics.DebuggerStepThrough()]
        private static void AssertionFail(string message, string argument, string callerMember, string sourcefile,int sourceline)
        {
            StackTrace trace1 = new StackTrace(true);
            GuardAssertException ex = null;
            if (string.IsNullOrEmpty(argument) == false)
            {
                message = string.Format("{1}{0}Argument: {2}", message, Environment.NewLine, argument).Trim();
            }
            ex = new GuardAssertException(message);
            if (string.IsNullOrEmpty(callerMember) == false)
                ex.Source = string.Format("{0} {1}:{2}", callerMember, sourcefile, sourceline);
            throw ex;
        }
        ...
        private static void AssertionFail(string message, string argument)
        {
            ...
            ex = new GuardAssertException(message)
            {
                ...
                Source = trace1.GetFrame(2).ToString()
            };
            throw ex;
        }
        ...
        [Conditional("ASSERT"), System.Diagnostics.DebuggerNonUserCodeAttribute(), System.Diagnostics.DebuggerStepThrough()]
        public static void AssertNotNull(object obj, [System.Runtime.CompilerServices.CallerMemberName] string memberName = "",
        [System.Runtime.CompilerServices.CallerFilePath] string sourceFilePath = "",
        [System.Runtime.CompilerServices.CallerLineNumber] int sourceLineNumber = 0)
        {
            if (IgnoreAssert) return;
            if (obj != null) return;
            AssertionFail("Assertion (Not Null) failed!",string.Empty, memberName, sourceFilePath,sourceLineNumber);
        }
